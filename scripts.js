...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Sprawdź czy są dane galerii w localStorage i aktualizuj portfolio
  updatePortfolioFromAdmin();
  ...
  if (portfolioContainer && portfolioItems.length > 0) {
    ...
    portfolioContainer.style.minHeight = `${gridHeight}px`;
  }
  ...
  const specialItems = document.querySelectorAll(
    ".portfolio-item.special, .portfolio-item.okolicznosciowe"
  );
  console.log(
    "Liczba elementów okolicznościowych (special/okolicznosciowe):",
    specialItems.length
  );
  console.log(
    ...
    button.addEventListener("click", function () {
          ...
          setTimeout(() => {
            if (filter === "special") {
              ...
              if (
                ...
                console.log(
                  "Ukrywam element (nie jest okolicznościowy):",
                  item.className
                );
              }
            } else if (!item.classList.contains(filter)) {
      ...
      setTimeout(() => {
        ...
        if (visibleItems.length > 0) {
          ...
          // Jeśli mamy elementy, oblicz rzeczywistą wysokość
          ...
          newHeight = newHeight - gridRect.top + 20; // Zmniejszony dodatkowy margines z 50px na 20px

          // Ustaw minimalną wysokość - zmniejszona z 500px na 350px
          portfolioContainer.style.minHeight = `${Math.max(newHeight, 350)}px`;
  ...
  if (testimonialsContainer && testimonialSlides.length > 0) {
    ...
    const cloneLast =
      testimonialSlides[testimonialSlides.length - 1].cloneNode(true);
  ...
  testimonialsContainer.style.transform = `translateX(-${
    slideWidth / (window.innerWidth >= 1024 ? 3 : 1)
  }%)`;
  ...
  function goToSlide(slideIndex, instant = false) {
    ...
    const translateValue =
      (-currentSlide * slideWidth) / (window.innerWidth >= 1024 ? 3 : 1);
  ...
  function animateCounter() {
    ...
    counters.forEach((counter) => {
      ...
      const updateCounter = () => {
        ...
        counter.textContent =
          current + (counter.getAttribute("data-suffix") || "");
      };
...
// Funkcja aktualizująca galerię portfolio na stronie głównej
function updatePortfolioFromAdmin() {
...
// Funkcja pomocnicza do dodawania elementów portfolio
...
function updatePortfolioFilters() {
  ...
  const updatedFilterButtons = document.querySelectorAll(
    ".portfolio-filter-btn"
  );

  // Domyślnie aktywuj przycisk "Wszystkie"
  const allButton = document.querySelector(
    '.portfolio-filter-btn[data-filter="all"]'
  );
  if (allButton) {
...
function initializeLightbox() {
  ...
  console.log(
    "Znaleziono",
    galleryItems.length,
    "zdjęć do inicjalizacji lightbox"
  );
